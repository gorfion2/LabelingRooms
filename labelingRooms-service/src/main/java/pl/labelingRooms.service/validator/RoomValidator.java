...
public class RoomValidator extends AbstractValidator<RoomDto> {
    ...
    public void validateRoomId(String roomId) throws InvalidDataException {
        if (roomId == null || repo.findOneByNumber(roomId)==null)
...
//            if (!repo.findOneByNumber(modelToEdit.getNumber()).getEventList().isEmpty())
//                throw new InvalidDataException("Pokój nie może zostać pokojem do konsultacji, ponieważ odbywają się w nim zajęcia");
//        } else {
//            if (!repo.findOneByNumber(modelToEdit.getNumber()).getTeacherList().isEmpty())
